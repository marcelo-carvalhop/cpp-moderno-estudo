...
*Benefícios do Padrão IIFE:*
  ...
  1. const Correctness: Permite que variáveis que exigem uma lógica de inicialização multi-etapas sejam declaradas como const, o que é uma prática recomendada de software.
  2. Encapsulamento: Toda a lógica de inicialização e quaisquer variáveis temporárias (como config no exemplo) ficam contidas dentro do escopo da lambda. Isso evita a poluição do escopo circundante.
  3. Localidade: O código de inicialização fica exatamente ao lado da variável que está sendo inicializada, melhorando drasticamente a legibilidade e a manutenção do código.

** 3* 32.2 Fábricas de Funções e std::function
...
A Solução (Usando std::function): A técnica mais comum e legível para criar uma lambda recursiva envolve std::function e a captura por referência.
  ...
  1. Declare um objeto std::function que representará a função recursiva.
  2. Defina a lambda, capturando o objeto std::function por referência.
  3. Atribua a lambda ao objeto std::function.
