...
A função ~std::async~ aceita uma política de lançamento como primeiro argumento, que dita como a tarefa será executada:
  ...
  1. ~std::launch::async~: Garante que a tarefa seja executada em uma nova thread separada. É o verdadeiro paralelismo.

  2. ~std::launch::deferred~: A tarefa não é executada imediatamente. Ela é executada de forma síncrona na thread atual apenas quando (e se) chamarmos ~.get()~ ou ~.wait()~ no futuro. É uma forma de "avaliação preguiçosa" (lazy evaluation).
