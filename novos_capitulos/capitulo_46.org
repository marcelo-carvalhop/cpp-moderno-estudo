...
As implicações disso são profundas:
    ...
    1. *Cascata de Compilação*: Se um arquivo de cabeçalho de baixo nível (~string_utils.h~) é modificado, todo e qualquer arquivo de implementação (.cpp) que o inclua, direta ou indiretamente, deve ser recompilado. Em um sistema com milhares de arquivos, uma mudança em um cabeçalho fundamental pode desencadear a recompilação de todo o projeto, transformando o tempo de build de segundos para horas.
    2. *Acoplamento Físico*: A inclusão de um cabeçalho (~#include "componente_A.h"~) cria uma dependência física explícita. Isso significa que o arquivo atual não pode ser compilado sem que o componente_A.h esteja disponível. Isso dita a estrutura do build e a ordem de compilação.
